...
test_input_allowed_deployment {
    input := { "review": input_deployment_review(input_container_allowed), "parameters": {"repos": ["allowed"]}}
    results := violation with input as input
    count(results) == 0
}

test_input_denied_deployment_x2 {
    input := { "review": input_deployment_review(input_container_denied), "parameters": {"repos": ["other", "allowed"]}}
    results := violation with input as input
    count(results) == 1
}
...
input_deployment_review(containers) = output {
    output = {
      "object": {
        "metadata": {
            "name": "nginx"
